...
def mockup_guidance(scenario_key: str | None) -> str:
    """design/applied/의 목업을 어디까지 반영해야 하는지 안내하는 문장.

    scenario_key가 없으면(전체 재작업/최초 구현) 모든 화면을 다 확인하라고
    하고, scenario_key가 있으면(이슈 하나로 범위를 좁힌 재작업) 그 화면만
    반영하고 다른 화면은 건드리지 말라고 한다 — OpenHands는 자유도 높은
    코딩 에이전트라 이건 강제가 아니라 유도일 뿐이라는 점에 유의."""
    if scenario_key:
        return (
            f"design/applied/{scenario_key}.html에 해당하는 화면만 반영하고, "
